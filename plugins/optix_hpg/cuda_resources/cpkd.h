...
MM_OPTIX_INTERSECTION_KERNEL(cpkd_treelet_intersect)() {
    const int treeletID = optixGetPrimitiveIndex();
    const auto& self = getProgramData<CTreeletsGeoData>();
...
MM_OPTIX_CLOSESTHIT_KERNEL(cpkd_treelets_closesthit)
() {
...
MM_OPTIX_CLOSESTHIT_KERNEL(cpkd_treelets_closesthit_occlusion)
() {
    optixSetPayload_0(1);
}
