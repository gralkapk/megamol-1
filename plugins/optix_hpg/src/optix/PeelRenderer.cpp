...
megamol::optix_hpg::PeelRenderer::PeelRenderer()
        : AbstractRenderer()
        , spp_slot_("spp", "")
...
void megamol::optix_hpg::PeelRenderer::setup() {
...
void megamol::optix_hpg::PeelRenderer::on_cam_param_change(
...
void megamol::optix_hpg::PeelRenderer::on_cam_pose_change(core::view::Camera::Pose const& cam_pose) {
    auto& frame_state = get_framestate();
...
void megamol::optix_hpg::PeelRenderer::on_change_data(OptixTraversableHandle world) {
    auto& frame_state = get_framestate();
...
void megamol::optix_hpg::PeelRenderer::on_change_background(glm::vec4 const& bg) {
    auto& frame_state = get_framestate();
...
void megamol::optix_hpg::PeelRenderer::on_change_programs(std::tuple<OptixProgramGroup const*, uint32_t> const& programs) {
    auto& frame_state = get_framestate();
    frame_state.frameIdx = 0;
...
void megamol::optix_hpg::PeelRenderer::on_change_parameters() {
    auto& frame_state = get_framestate();
...
void megamol::optix_hpg::PeelRenderer::on_change_viewport(
    ...
    frame_state.frameIdx = 0;
    if (ray_buffer_ptr) {
        CUDA_CHECK_ERROR(cuMemFreeAsync(ray_buffer_ptr, get_context()->GetExecStream()));
    }
    CUDA_CHECK_ERROR(cuMemAllocAsync(
        &ray_buffer_ptr, sizeof(device::Ray) * viewport.x * viewport.y, get_context()->GetExecStream()));
    frame_state.rayBuffer = (device::Ray*)ray_buffer_ptr;
    if (hp_treelet_requests_ptr_) {
        CUDA_CHECK_ERROR(cuMemFreeHost(hp_treelet_requests_ptr_));
    }
    CUDA_CHECK_ERROR(cuMemHostAlloc(
        (void**) &hp_treelet_requests_ptr_, sizeof(glm::ivec2) * viewport.x * viewport.y, CU_MEMHOSTALLOC_DEVICEMAP));
    new (hp_treelet_requests_ptr_) glm::ivec2[viewport.x * viewport.y];
    CUdeviceptr d_treelet_requests_ptr = 0;
    CUDA_CHECK_ERROR(cuMemHostGetDevicePointer(&d_treelet_requests_ptr, hp_treelet_requests_ptr_, 0));
    frame_state.treeletRequests = (glm::ivec2*)d_treelet_requests_ptr;
}


void megamol::optix_hpg::PeelRenderer::on_change_sbt(std::tuple<void const*, uint32_t, uint64_t> const& records) {
...
void megamol::optix_hpg::PeelRenderer::on_launch_func() {
    // gather treelet requests
    auto& frame_state = get_framestate();
    auto const pixCount = sbt_raygen_record_.data.fbSize.x * sbt_raygen_record_.data.fbSize.y;
    /*std::vector<glm::ivec2> treeletRequests(pixCount);
    CUDA_CHECK_ERROR(cuMemcpyDtoHAsync(treeletRequests.data(), (CUdeviceptr) frame_state.treeletRequests,
        pixCount * sizeof(int), get_context()->GetExecStream()));*/
    CUDA_CHECK_ERROR(cuStreamSynchronize(get_context()->GetExecStream()));
    auto last_el = std::unique(
        hp_treelet_requests_ptr_, hp_treelet_requests_ptr_ + pixCount, [](auto const& lhs, auto const& rhs) {
            int64_t l = lhs.x;
            l <<= 32;
            l += lhs.y;
            int64_t r = rhs.x;
    ...
    // only missing treelets are reported

    ZeroMemory(hp_treelet_requests_ptr_, pixCount * sizeof(glm::ivec2));
    /*CUDA_CHECK_ERROR(
        cuMemsetD32Async((CUdeviceptr) frame_state.treeletRequests, 0, pixCount, get_context()->GetExecStream()));*/
}
