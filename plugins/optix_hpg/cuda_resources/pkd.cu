...
inline __device__ bool clipToBounds(const Ray& ray, const glm::vec3& lower, const glm::vec3& upper, float& t0, float& t1) {
    glm::vec3 t_lower = (lower - ray.origin) / ray.direction;
    glm::vec3 t_upper = (upper - ray.origin) / ray.direction;
...
MM_OPTIX_INTERSECTION_KERNEL(pkd_intersect)() {
    ...
    float t0, t1;
    if (!clipToBounds(ray, self.worldBounds_lower, self.worldBounds_upper, t0, t1))
        return;
