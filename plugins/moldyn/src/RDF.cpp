...
RDF::RDF(std::shared_ptr<std::vector<glm::vec3>> org_data, std::shared_ptr<std::vector<glm::vec3>> new_data)
        : org_data_{org_data}
        , new_data_{new_data} {
    auto t1 = std::thread([this, org_data_adr = &org_data]() {
        this->org_Pts_ = std::make_shared<GLMPointcloud>(org_data_adr->get());
    ...
    auto t2 = std::thread([this, new_data_adr = &new_data]() {
        this->new_Pts_ = std::make_shared<GLMPointcloud>(new_data_adr->get());
...
std::vector<float> compute_histo(float cut_off, unsigned int num_bins, std::shared_ptr<std::vector<glm::vec3>> data,
...
#pragma omp parallel for reduction(+ : num_samples)
    for (int64_t i = 0; i < data->size(); ++i) {
        ...
        auto const N = tree->radiusSearch(glm::value_ptr(od), cut_off * cut_off, results[threadID], params);
        if (N > 1) {
            num_samples += N - 1;
